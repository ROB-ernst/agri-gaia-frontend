...
export default function (props: {tasksList: IBackgroundTask[], onDelete: (task: IBackgroundTask) => void, rowHeight: number, emptyText?: string, onTaskDetails: (task: IBackgroundTask) => void}) {
            ...
            {hasTasks ? tasksList.map((task) => {
                return (
                    <Box key={task.id}>
                        ...
                        {task.id !== tasksList[tasksList.length - 1].id && (
                            <Divider />
                        )}
                    </Box>
                );
